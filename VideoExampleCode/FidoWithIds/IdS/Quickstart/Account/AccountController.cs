...
namespace IdentityServer4.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        private IEnumerable<Claim> BuildClaims(LoginInputModel model)
        {
            var claims = new List<Claim>();

            var user = _users.FindByUsername(model.Username);

            claims.Add(new Claim("rememberme", (AccountOptions.AllowRememberLogin && model.RememberLogin).ToString()));
            claims.Add(new Claim("redirectUrl", model.ReturnUrl));
            claims.Add(new Claim("userName", user.Username));
            claims.Add(new Claim("subjectId", user.SubjectId));
        ...
        public IActionResult StartRegistration()
        {
            ...
            {
                return RedirectToAction("Login", new {ReturnUrl = "/account/startregistration"});
            }
        }
        ...
        public async Task<IActionResult> Register(RegistrationModel model)
        {
            ...
            var viewModel = new RegistrationViewModel {FidoRegistrationChallenge = challenge.ToBase64Dto()};
        ...
        private static string GeneratorNonce()
        {
